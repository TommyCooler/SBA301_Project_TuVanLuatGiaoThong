...
export default function LawManagementSection() {
  ...
  useEffect(() => {
  console.log('Law types updated:', lawTypes);
  // Log các giá trị cụ thể để debug
  if (lawTypes.length > 0) {
    console.log('First law type:', lawTypes[0]);
  }
}, [lawTypes]);
  ...
  const handleEdit = (law: Law) => {
    ...
    // Format ngày trước khi đặt vào formData
    const formattedLaw = {
      ...law,
      // Chuyển định dạng ngày từ API thành định dạng "YYYY-MM-DD" cho input type="date"
      issueDate: law.issueDate ? law.issueDate.split('T')[0] : '',
      effectiveDate: law.effectiveDate ? law.effectiveDate.split('T')[0] : ''
    };
    
    setFormData(formattedLaw);
  ...
  return (
    <div className="p-6">
            ...
            <table className="min-w-full divide-y divide-gray-200 table-fixed">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {laws.map((law) => (
                  <tr 
                    key={law.id} 
                    className={`hover:bg-gray-50 ${law.deleted ? 'bg-gray-100 opacity-60' : ''}`}
                  >
                    <td className="px-6 py-4 max-w-xs truncate" title={law.title}>
                      {law.title}
                      {law.deleted && <span className="ml-2 text-xs py-1 px-2 bg-red-100 text-red-600 rounded-full">Đã xóa</span>}
                    </td>
                      ...
                      <div className="flex gap-2">
                        {law.filePath && (
                          <a
                            ...
                            className={`text-blue-600 hover:text-blue-900 ${law.deleted ? 'opacity-50' : ''}`}
                            title="Xem PDF"
                          >
      ...
      {/* Modal */}
      {isModalOpen && (
        <div className="fixed inset-0 backdrop-blur-sm bg-white/30 flex items-start justify-center overflow-y-auto pt-10 pb-10">
          <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-xl my-auto max-h-[90vh] overflow-y-auto">
