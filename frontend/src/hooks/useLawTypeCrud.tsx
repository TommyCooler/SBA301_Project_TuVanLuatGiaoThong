...
export function useLawTypeCrud() {
    // Vẫn giữ api từ useAxios cho các phương thức khác nếu cần
    const api = useAxios();
    ...
    // Get all law types - sử dụng axios trực tiếp
    const getAllLawTypes = useCallback(async () => {
        setLoading(true);
        setError(null);
        try {
            const response = await axios.get(Api.BASE_API + Api.LawType.GET_ALL);
    ...
    // Get law type by ID - sử dụng axios trực tiếp
    const getLawTypeById = useCallback(async (id: string) => {
        setLoading(true);
        setError(null);
        try {
            const response = await axios.get(Api.BASE_API + Api.LawType.GET_BY_ID + id);
            if (response.status === HttpStatus.OK) 
    ...
    // Update law type
    const updateLawType = useCallback(async (id: string, updatedLawType: Partial<LawType>) => {
        setLoading(true);
        setError(null);
        try {
            const response = await api.put(Api.LawType.UPDATE + id, updatedLawType)
            if (response.status === HttpStatus.OK) {
                setLawTypes((prev) => prev.map((lt) => (lt.id === id ? response.data.dataResponse : lt)));
    ...
    const deleteLawType = useCallback(async (id: string) => {
        ...
        setError(null);
        try {
            const response = await api.put(Api.LawType.UPDATE + id, { isDeleted: true })
            if (response.status === HttpStatus.OK) {
